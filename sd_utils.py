...
from pix2pix_zero.src.utils.base_pipeline import BasePipeline
from pix2pix_zero.src.utils.cross_attention import prep_unet
...
class ReeditingPipeline(BasePipeline):
    def __call__(
        self,
    ...
    def edit(
        ...
        with self.progress_bar(total=self.num_inference_steps) as progress_bar:
            for i, t in enumerate(self.timesteps):
                ...
                # predict the noise residual
                noise_pred = self.unet(x_in,t,encoder_hidden_states=prompt_embeds_edit.detach(),cross_attention_kwargs=self.cross_attention_kwargs,).sample
                ...
                # recompute the noise
                with torch.no_grad():
                    noise_pred = self.unet(x_in.detach(),t,encoder_hidden_states=prompt_embeds_edit,cross_attention_kwargs=self.cross_attention_kwargs,).sample
